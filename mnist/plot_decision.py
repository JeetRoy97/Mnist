...
def validate(data, X_val, y_val,i, s,h,clf):
    ...
    if accuracy < 0.11:
        pass
        print("Skipping for rescale {}x{} testsize {} depth {}". format(int(math.sqrt(data.shape[1])), int(math.sqrt(data.shape[1])), s, i))
    candidate = {
        'acc_valid' : accuracy,
        'depth' : i,
        'split' : s,
...
def classification_model_dt(data, X_train, y_train, X_val, y_val, depth, split, shape,  model_path):
    #model_candidate = []
    clf = tree.DecisionTreeClassifier(max_depth = depth)
    clf.fit(X_train, y_train) 
    model_candidate = validate(data, X_val, y_val,depth, split, int(math.sqrt(data.shape[1])),clf) # validation function
    #print(model_candidate) 
    output = model_path+"s_{}_tt_{}_val_{}_depth{}".format(int(math.sqrt(data.shape[1])),split, split, depth)
    os.makedirs(output)
...
best_model_folder = "/home/jeet/MLOPs/Mnist/mnist/models_dt/s_{}_tt_{}_val_{}_depth{}".format(max_valid['shape'],max_valid['split'], max_valid['split'], max_valid['depth'])
